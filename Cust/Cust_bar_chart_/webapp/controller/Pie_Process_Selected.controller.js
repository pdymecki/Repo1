sap.ui.define([
		...
		return BaseController.extend("custom.controller.Pie_Process_Selected", {
			...
			this._oRouter.getRoute("pie_process_selected").attachMatched(this.page_refresh, this);
			...
			var ba_selected= sap.ui.getCore().getModel("process_selected");
		...
		//	alert(ba_selected);
			...
			for(var i=0;i<pi_data.length;i++){
				if(pi_data[i].ProcessName.length>0&&pi_data[i].ProcessName==ba_selected.process){
		...
		var selPiefn=
			...
			bla: function(event){
				...
				var name = new Object();
				name.ba=data[0].data.name;
				var j_model = sap.ui.model.json.JSONModel();
		...
		oModel.setData(pi_object);
			...
			oVizFrame.setVizProperties({
			title:{
				text : "Process Steps for "+ba_selected.process
			},
            plotArea: {
            //	colorPalette : d3.scale.category20().range(),
            //	drawingEffect: "glossy"
